 ...
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.pi.sh`). Each
 ...
 * `~/.pi/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `pi grievances push`, future debug scripts —
 ...
 *
 * Exported because the `pi grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `pi grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`pi grievances push`) where the
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.pi/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `pi grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `pi grievances push` or a future consent change.
