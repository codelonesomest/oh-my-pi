...
export class CmuxTab {
	...
	async #responseCursor(): Promise<number> {
		const value = await this.#evalScript<unknown>(
			"(() => Math.max(0, ((globalThis.__piCmuxResponses && globalThis.__piCmuxResponses.nextId) || 1) - 1))()",
		);
	...
	async #responseRecordsAfter(id: number): Promise<CmuxResponseRecord[]> {
		const value = await this.#evalScript<unknown>(
			`(() => ((globalThis.__piCmuxResponses && globalThis.__piCmuxResponses.records) || []).filter(record => record.id > ${JSON.stringify(id)}))()`,
		);
