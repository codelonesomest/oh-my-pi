...
// `__pi_import__` helper, which resolves the specifier against the active session cwd. A real
...
// worker-injected `__pi_import__` global does not exist. The swap therefore guards on the
...
const DYNAMIC_IMPORT_CALLEE = '(typeof __pi_import__ === "function" ? __pi_import__ : (s, o) => import(s, o))';

function buildOmpImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	// Route every static import through the worker-injected `__pi_import__` helper so the
	// specifier resolves against the session cwd (and `with`-attribute imports keep working).
	return optionsLiteral ? `__pi_import__(${sourceLiteral}, ${optionsLiteral})` : `__pi_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__pi_import__` forwards the
...
export async function rewriteImports(code: string): Promise<string> {
	...
	// Top-level static `import` declarations become `await __pi_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__pi_import__`
...
export async function rewriteDynamicImports(code: string, callee = "__pi_import__"): Promise<string> {
	if (!code.includes("import")) return code;
	const ast = await parseProgram(code);
	if (!ast) return code;
...
async function returnFinalExpression(code: string): Promise<{ source: string; returned: boolean }> {
	...
	if (last?.type === "ExpressionStatement") {
		...
		return { source: `${prefix}__pi_set_final_expr__((${trimmedStatement}));${suffix}`, returned: true };
	}
	if (last?.type === "ReturnStatement") {
		// Top-level `return value;` is otherwise swallowed: it forces the cell into an async IIFE
		// wrapper that discards the returned value. Rewrite into `__pi_set_final_expr__((expr))`
		...
		return { source: `${prefix}__pi_set_final_expr__((${expr}));${suffix}`, returned: true };
	}
