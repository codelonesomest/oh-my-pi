 ...
 * SSH JSON Provider
 *
 * Discovers SSH hosts from managed pi config paths and legacy root ssh.json files.
...
registerProvider(sshCapability.id, {
	...
	description: "Load SSH hosts from managed pi paths and legacy ssh.json/.ssh.json files",
	priority: 5,
	load,
});
