...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ pi?: ExtensionModuleManifest }>(content);
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with `pi` field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.pi/`.
 *    The first match returns `<dir>/.pi/plugins/installed_plugins.json`.
 * 2. If no `.pi/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.pi/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .pi/ directory (nearest wins).
	// Stop before os.homedir() — ~/.pi/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.pi/plugins/installed_plugins.json`
 * when no project anchor (.pi/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .pi/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.pi/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// Pi registry is authoritative: its entries replace Claude's entries for the same plugin ID.
