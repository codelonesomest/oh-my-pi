...
/**
 * Read `PI_NO_WEBP` (or legacy `OMP_NO_WEBP`) per-call so runtime toggles take effect.
 * Only `"1"` and `"true"` (case-insensitive) enable exclusion; `"0"` and `"false"`
 * disable it. Empty `PI_NO_WEBP` falls through to the legacy fallback.
 */
function isWebPExcluded(): boolean {
	const raw = Bun.env.PI_NO_WEBP?.trim() || Bun.env.OMP_NO_WEBP?.trim();
 ...
 *
 * Set PI_NO_WEBP (or legacy OMP_NO_WEBP) to exclude WebP from encoding (llama.cpp STB doesn't decode it).
 *
...
export async function resizeImage(img: ImageContent, options?: ImageResizeOptions): Promise<ResizedImage> {
	...
	try {
		...
		// WebP usually beats JPEG by 25–35% but is disabled when PI_NO_WEBP (or legacy OMP_NO_WEBP) is set
		...
		// unless PI_NO_WEBP or legacy OMP_NO_WEBP is set (llama.cpp STB incompatibility).
