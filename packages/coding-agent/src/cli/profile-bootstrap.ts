 ...
 * doesn't look like another flag. Without this, `pi --system-prompt --profile
 ...
 * `pi --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`pi --print --profile work`).
 */
 ...
 * positionals are never stolen (`pi grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `pi launch --profile work` and
 * `pi acp --profile work` still select profile `work`.
 *
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `pi --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`pi --some-ext-bool --profile work`
		...
		// activates (`pi --print --profile work`).
