/**
 * `pi models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.pi/models.db`
 ...
 * name doubles as an `pi models <provider>` shortcut.
 */
...
/** `pi models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/** `pi models canonical`: the coalesced canonical view (one row per canonical id). */
...
/**
 * Entry point for the standalone `pi models` command: bootstraps auth storage,
