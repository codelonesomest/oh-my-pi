 ...
 * User-level: ~/.pi/agent, ~/.claude, ~/.codex, ~/.gemini
 * Project-level: .pi, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".pi", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.pi/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.pi/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.pi/agent/commands", source: ".pi", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.pi, .claude) - the nearest one found.
