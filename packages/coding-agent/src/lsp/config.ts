...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.pi/, .claude/)
	...
	// User config directories (~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
