...
export const SETTINGS_SCHEMA = {
	...
	// Auth broker — credentials proxied through a remote `pi auth-broker serve`
	// host. Hidden from the UI; populate via env vars or hand-edited config.yml.
	// Env (`PI_AUTH_BROKER_URL` / `PI_AUTH_BROKER_TOKEN`, with legacy OMP fallback)
	// takes precedence so per-machine overrides remain trivial.
	...
	"startup.checkUpdate": {
		...
		ui: {
			...
			description: "Check for pi updates on startup",
		},
	},
	...
	// and mint/enhance isolated managed skills under ~/.pi/agent/managed-skills.
	...
	"worktree.base": {
		...
		ui: {
			...
			description:
				"Base directory for agent-managed worktrees — task-isolation copies, `github` PR checkouts, and `pi worktree` cleanup all live here. Unset uses ~/.pi/wt. Must be an absolute or ~-relative path; relative paths are ignored. The PI_WORKTREE_DIR env var overrides this; legacy OMP_WORKTREE_DIR is also accepted as a fallback.",
		},
	},
	...
	"dev.autoqaPush.endpoint": {
		type: "string",
		default: "https://qa.pi.sh/v1/grievances" as const,
		ui: {
			tab: "tools",
			group: "Developer",
			label: "Auto QA Push Endpoint",
			description: "Full URL receiving Auto QA JSON reports (default https://qa.pi.sh/v1/grievances)",
		},
	},
