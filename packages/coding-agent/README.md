...
Core implementation package for the `pi` coding agent in the `oh-my-pi` monorepo.
...
The agent supports three mutually-exclusive memory backends, selected via the `memory.backend` setting (Settings → Memory tab, or `~/.pi/agent/config.yml`):
