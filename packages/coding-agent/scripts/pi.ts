/**
 * Bun `--preload` shim for the pi dev launcher (`scripts/pi`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the pi CLI: Bun reads
...
if (launchCwd) {
	delete process.env.PI_LAUNCH_CWD;
	try {
