#!/usr/bin/env bun
/**
 * Build the standalone share-viewer page the pi relay serves at `GET /s/<id>`.
 *
 ...
 * hands the JSON to template.js via `window.__PI_SESSION_DATA__`.
 *
...
// Pin the pi brand palette (collab-web pink/purple identity) — the viewer is
// a public artifact matching the live my.pi.sh client, not a per-user export
...
const html = getTemplate()
	...
	.replace("<title>Session Export</title>", () => "<title>pi session</title>")
...
if (!html.includes("__PI_SESSION_DATA__")) throw new Error("share loader not injected");
