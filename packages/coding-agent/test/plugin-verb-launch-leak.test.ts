/**
 * Regression test for #2935: the plugins docs advertise `pi list` / `pi remove`
 * as top-level commands, but only `pi install` is registered. Before the fix,
 * `resolveCliArgv(["list"])` rewrote the bare verb to `["launch", "list"]`, so
 * `pi list` silently started an interactive agent session with "list" as the
 * initial LLM prompt instead of managing plugins (the real command is
 * `pi plugin list`). Same footgun for `pi remove`.
 *
 * These tests pin the chosen bugfix: a bare, single-arg documented plugin verb
 * yields a helpful hint pointing at the real `pi plugin <action>` command
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `pi list` hints at `pi plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("pi plugin list");
	});

	test("bare `pi remove` hints at `pi plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("pi plugin uninstall");
	});
