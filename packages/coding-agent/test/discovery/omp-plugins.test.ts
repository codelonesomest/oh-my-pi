 ...
 *
 * The native `pi` discovery provider only walks `.pi/` and `~/.pi/agent/`.
...
function buildExtensionPackage(packageDir: string): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), pi: { extensions: ["./src/main.ts"] } }),
	);
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `pi plugin link`/`install`)", async () => {
	...
	// Plugin's own package.json must carry a `pi` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `pi.extensions`, which is sufficient.
...
test("project-scoped installed plugins surface project-level sub-discovery", async () => {
	...
	fs.cpSync(ext, installed, { recursive: true });
	writeFile(
		path.join(pluginsDir, "pi-plugins.lock.json"),
		JSON.stringify({
...
test("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	// `pi plugin link ./local-ext` creates a symlink under
	...
	// still find the package — otherwise the documented `pi install
