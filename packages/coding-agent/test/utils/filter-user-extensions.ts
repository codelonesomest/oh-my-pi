...
//    or `../`-relative entry pointing somewhere else under `~/.pi/`), plus
//    the legacy non-XDG `~/.pi/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.pi/agent` or `$XDG_CONFIG_HOME`)
//    Handles `XDG_CONFIG_HOME` overrides that relocate the agent dir
//    (and therefore `agent/extensions/`) out from under the config root.
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/pi/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.pi` when
...
// symlinks `~/.pi/agent/extensions/foo.ts` to a dotfiles repo would see the
