...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-proj-scope-"));
	});
	...
	it("walk-up finds nearest .pi/ directory", async () => {
		// Layout: tmpDir/.pi/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .pi/ there.
		...
		expect(result).toBe(path.join(tmpDir, ".pi", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .pi/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.pi/   +   tmpDir/sub/.pi/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.pi/, not climb further to tmpDir/.pi/.
		fs.mkdirSync(path.join(tmpDir, ".pi"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".pi"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".pi", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .pi/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .pi/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .pi/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .pi/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.pi/.../installed_plugins.json.
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-proj-scope-home-"));
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homePiPath = path.join(homeDir, ".pi", "plugins", "installed_plugins.json");
			expect(result).not.toBe(homePiPath);
			expect(result).toBeNull();
		} finally {
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user PI registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "pi-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "pi-shadow-proj-"));

		// Create .pi/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".pi", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".pi", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".pi", "plugins", "installed_plugins.json");
	});
