...
describe("executeJs", () => {
	...
	it("does not expose the final expression marker as a global property", async () => {
		...
		const marker = await executeJs("return Object.hasOwn(globalThis, '__pi_final_expr__');", {
			sessionId,
			session,
			sessionFile,
		});
	...
	it("ignores user-assigned final expression markers without a rewritten final expression", async () => {
		const result = await executeJs("globalThis.__pi_final_expr__ = 'manual'; return 'actual';", {
			sessionId,
			session,
			sessionFile,
		});
	...
	it("captures promise-valued final expression before promise callbacks can mutate the marker", async () => {
		const result = await executeJs(
			"const pending = Promise.resolve(1).then(value => { globalThis.__pi_final_expr__ = 999; return value; }); pending;",
			{ sessionId, session, sessionFile },
		);
