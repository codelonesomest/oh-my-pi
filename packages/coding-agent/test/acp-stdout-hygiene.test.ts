/**
 * ACP stdout-hygiene smoke: launching `pi acp` must not leak any banner,
 ...
 * unwind, but force-kill quickly if it hasn't reaped — `pi acp` blocks on
...
describe("ACP stdout hygiene", () => {
	it("emits a JSON-RPC initialize response as the first bytes on stdout", async () => {
		...
		// run (~12s cold vs ~0.4s warm). XDG_*
