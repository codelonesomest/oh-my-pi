...
describe("advisor watchdog prompt discovery", () => {
	...
	it("discovers user-level and native project-level watchdog files", async () => {
		...
		fs.writeFileSync(path.join(piDir, "WATCHDOG.md"), nativeWatchdogContent, "utf8");
		...
		try {
			...
			// Check ordering: user-level should appear first, then native project level (.pi/WATCHDOG.md has depth 0),
