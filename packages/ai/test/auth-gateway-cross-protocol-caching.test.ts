 ...
 *     → openai-responses parser → pi Context
 ...
 * (override via `PI_E2E_GATEWAY_URL`) AND the bearer token file exists at
 * `~/.pi/auth-gateway.token`.
 *
 * To run: `bun --cwd packages/ai test test/auth-gateway-cross-protocol-caching.test.ts`
 * with the gateway live (`pi auth-gateway serve` or pm2).
 */
...
the pi auth-gateway's cross-protocol prompt-caching pipeline. The request
