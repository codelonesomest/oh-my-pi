			...
			channel_values: {
				ui_chat_log: [
					{ message_type: "user", content: "Question" },
					{
						...
						tool_info: { name: "mcp__pi__read", args: { path: "src/index.ts" } },
					},
			...
			channel_values: {
				ui_chat_log: [
					...
					{ message_type: "agent", content: "Found the target. Reading it now." },
					{
						...
						tool_info: { name: "mcp__pi__read", args: { path: "README.md" } },
					},
					{
						message_type: "tool",
						content: "README text",
	...
	it("emits standard tool calls instead of executing GitLab actions in the provider", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-mcp-1",
					runMCPTool: { name: "mcp__pi__read", args: JSON.stringify({ path: "src/index.ts" }) },
				}),
			}),
		);
	...
	it("finalizes one assistant message per tool-call action (serial MCP dispatch)", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-a",
					runMCPTool: { name: "mcp__pi__read", args: JSON.stringify({ path: "a.ts" }) },
				}),
			}),
		);
		...
		socket?.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					...
					runMCPTool: { name: "mcp__pi__read", args: JSON.stringify({ path: "README.md" }) },
				}),
			}),
		);
