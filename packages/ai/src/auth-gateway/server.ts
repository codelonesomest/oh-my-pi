 ...
 * pure protocol translation: foreign wire → pi Context → pi-ai stream() →
 * pi events → foreign wire.
 *
 * Endpoints:
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (pi re-stamps every parsed Message). role +
