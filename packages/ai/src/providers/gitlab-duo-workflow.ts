...
function mapGitLabDuoWorkflowMcpToolCall(args: Record<string, unknown>): {
	...
	const toolName = rawName.startsWith("mcp__pi__") ? rawName.slice("mcp__pi__".length) : rawName;
...
function buildGitLabMcpToolDefinition(tool: Tool): GitLabMcpToolDefinition {
	...
	// Register the tool under its BARE name (no `mcp__pi__` prefix). The server does
	...
	// model to learn `mcp__pi__read` while OMP docs say `read`, with no upside.
