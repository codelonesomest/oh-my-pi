 ...
 * build-time native artifacts), the compiled `pi` binary fails to load
 ...
 * `execDir`. On WSL with `~/.local/bin/pi` and no sibling `.node` file, this
...
describe("issue 823: standalone-binary native loader path resolution", () => {
	...
	it("places embedded-extracted candidates ahead of build-host candidates for linux-x64 standalone", () => {
		const versionedDir = "/home/u/.pi/natives/14.5.2";
		...
		// these are where the embedded-addon extraction lands (~/.pi/natives/<v>) and where
		// `pi update` writes the standalone binary on linux (~/.local/bin).
	...
	it("prefers platform leaf package candidates ahead of core nativeDir candidates on npm installs", () => {
		...
		const candidates = resolveLoaderCandidates({
			...
			versionedDir: "/home/u/.pi/natives/15.5.15",
	...
	it("keeps Windows staging ahead of leaf package and core nativeDir candidates", () => {
		const versionedDir = "/home/u/.pi/natives/15.5.15";
