/**
 * Regression for the Windows `bun install -g` update path: when an `pi`
 ...
 *      (per-package-version cache under `~/.pi/natives/<version>/`) so the
...
describe("windows native addon staging", () => {
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.pi/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
