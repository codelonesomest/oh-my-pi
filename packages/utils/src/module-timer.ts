 ...
 * - **Dev runs only.** In the compiled `pi` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("pi.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("pi.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("pi.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("pi.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
