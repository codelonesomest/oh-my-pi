...
```text
Config sources (.pi/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.pi/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.pi/mcp.json` (project) or `~/.pi/agent/mcp.json` (user) for explicit control.
...
Pi-native MCP config (`.pi/mcp.json`, `~/.pi/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
