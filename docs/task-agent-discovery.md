...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from Pi-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the Pi task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".pi"` filters both dir lists).

### Discovery inputs

1. Nearest project `.pi` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.pi`; first hit only)
2. User `.pi` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.pi`; first hit only)
