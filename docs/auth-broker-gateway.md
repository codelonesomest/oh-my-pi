...
- **`pi auth-broker serve`** holds the canonical SQLite credential vault, performs OAuth refreshes, and exposes a small REST API (`/v1/snapshot`, `/v1/snapshot/stream`, `/v1/credential/:id/refresh`, `/v1/credential/:id/disable`, `/v1/credential`, `/v1/usage`, `/v1/healthz`).
- **`pi auth-gateway serve`** is a forward-proxy. It accepts OpenAI Chat Completions, Anthropic Messages, OpenAI Responses, and pi-native stream requests, resolves the broker-backed credential, and dispatches through `pi-ai` provider logic. Clients (containerised pi, llm-git, the macOS usage widget, …) never see the access token.
...
```
                ...
                │                                                            │
  developer ──▶ │  ┌──────────────────────────┐    ┌────────────────────┐    │
  laptop /      │  │  pi auth-broker serve    │◀──▶│  SQLite agent.db    │    │
  CI / robomp   │  │  - holds refresh tokens  │    │  (canonical writer)│    │
                ...
                │            ▼                                               │
                │  ┌──────────────────────────┐                              │
                │  │  pi auth-gateway serve    │  RemoteAuthCredentialStore   │
...
```
pi auth-broker serve     [--bind=host:port]                    # boot the broker
pi auth-broker token     [--regenerate] [--json]               # print or rotate the bearer token
pi auth-broker login     [<provider>] [--via=user@host] [--dry-run]
pi auth-broker logout    [<provider>]
pi auth-broker list      [--json]
pi auth-broker import    <file|dir> [--provider=<id>] [--include-disabled] [--dry-run] [--json]
pi auth-broker migrate   --from-local [--include-oauth] [--include-env] [--dry-run] [--json]
pi auth-broker status    [--json]
```
...
- `import <file|dir>` imports CLIProxyAPI-style JSON credentials into the local SQLite store. Maps `type` field → pi provider (`claude → anthropic`, `codex → openai-codex`, `gemini → google-gemini-cli`, `antigravity → google-antigravity`, `gemini-cli → google-gemini-cli`).
...
```
pi auth-gateway serve   [--bind=host:port] [--no-auth]
pi auth-gateway token   [--regenerate] [--json]
pi auth-gateway status  [--json]
pi auth-gateway check   [--strict] [--json]
```

- `serve` requires `PI_AUTH_BROKER_URL` (or legacy `OMP_AUTH_BROKER_URL`, or `auth.broker.url` in `config.yml`) — the gateway is itself a broker client. It calls `AuthBrokerClient.fetchSnapshot()`, wraps it in `RemoteAuthCredentialStore`, and constructs an `AuthStorage` that resolves access tokens through the broker. Default bind is `127.0.0.1:4000`. The gateway token is stored at `<config-dir>/auth-gateway.token` (`0600`); `--no-auth` disables the bearer check entirely (loopback-only use).
...
The model id is read from the top-level `model` field for foreign wire formats and from the pi-native request body for `/v1/pi/stream`. The gateway picks the first bundled `Model<Api>` matching that id, parses the inbound wire format into a pi `Context`, resolves the provider credential from broker-backed `AuthStorage`, dispatches through `streamSimple()`, and re-encodes the result to the inbound format (SSE for streamed responses).
...
`discoverAuthStorage()` persists the broker snapshot to `~/.pi/cache/auth-broker-snapshot.enc` after the initial `/v1/snapshot` fetch and after later broker-sourced full snapshots. The file is AES-256-GCM encrypted with `SHA-256(<resolved broker token>)` and authenticated with the broker URL as additional data, so changing either the token or URL makes the cache unreadable. The file is written atomically with mode `0600`.

Freshness is anchored to the broker-stamped `snapshot.generatedAt`, not local write time. Default TTL is 1 h (`PI_AUTH_BROKER_SNAPSHOT_TTL_MS`; legacy fallback `OMP_AUTH_BROKER_SNAPSHOT_TTL_MS`); `0` disables the cache and restores the old always-fetch boot path. When the cached snapshot is still fresh, `pi` boots from it and skips the blocking `/v1/snapshot` query. `RemoteAuthCredentialStore` still starts its normal SSE / long-poll background sync immediately, so deleted or rotated credentials reconcile after startup, and expired OAuth access tokens still refresh through `POST /v1/credential/:id/refresh`.
...
The broker is **off** unless `PI_AUTH_BROKER_URL` (legacy fallback `OMP_AUTH_BROKER_URL`) or `auth.broker.url` in `config.yml` is set. When set, `discoverAuthStorage` in `packages/coding-agent/src/sdk.ts` swaps the local SQLite credential store for `RemoteAuthCredentialStore` and every API call resolves credentials through the broker.

### Environment variables

| Variable                           | Purpose                                                                                                                                            | Required when                                                                                                            |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `PI_AUTH_BROKER_URL`               | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`). Selecting this puts the client in broker mode — local SQLite is bypassed. | Any time the pi client should resolve credentials through a broker (and required by `pi auth-gateway serve`).          |
| `PI_AUTH_BROKER_TOKEN`             | Bearer token used for every broker endpoint except `/v1/healthz`.                                                                                  | When `PI_AUTH_BROKER_URL` is set and no token is available from `auth.broker.token` or `<config-dir>/auth-broker.token`. |
| `PI_AUTH_BROKER_SNAPSHOT_TTL_MS`   | Freshness window for the encrypted local snapshot cache. Default `3600000` (1 h); `0` disables cache reads and writes. Legacy fallback: `OMP_AUTH_BROKER_SNAPSHOT_TTL_MS`. | Optional in broker mode. |
| `PI_AUTH_BROKER_SNAPSHOT_CACHE`    | Path override for the encrypted local snapshot cache. Default `~/.pi/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Legacy fallback: `OMP_AUTH_BROKER_SNAPSHOT_CACHE`. | Optional in broker mode. |

Resolution order in `resolveAuthBrokerConfig()`:

1. `PI_AUTH_BROKER_URL` env (legacy fallback `OMP_AUTH_BROKER_URL`; else `auth.broker.url` from `config.yml`, resolved through `resolveConfigValue`);
2. `PI_AUTH_BROKER_TOKEN` env (legacy fallback `OMP_AUTH_BROKER_TOKEN`; else `auth.broker.token` from `config.yml`, else `<config-dir>/auth-broker.token`);
3. URL set but no token resolvable → hard error pointing at the token file path.

The gateway has no dedicated env vars — it inherits `PI_AUTH_BROKER_*` (`OMP_AUTH_BROKER_*` still work as legacy fallbacks) because it is itself a broker client.

### `config.yml` keys

| Key                 | Default | Purpose                                                                                                                                                                            |
| ------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auth.broker.url`   | unset   | Same as `PI_AUTH_BROKER_URL`; env wins. Hidden from the settings UI. Values are resolved as a literal, an environment variable name, or `!<shell command>` to use trimmed stdout. |
| `auth.broker.token` | unset   | Same as `PI_AUTH_BROKER_TOKEN`; env wins. Values are resolved the same way.                                                                                                       |

### Token files

| Path                              | Owner                                                | Mode                          |
| --------------------------------- | ---------------------------------------------------- | ----------------------------- |
| `<config-dir>/auth-broker.token`  | `pi auth-broker serve` (created at first start)     | `0600` in a `0700` parent dir |
| `<config-dir>/auth-gateway.token` | `pi auth-gateway serve` (skipped under `--no-auth`) | `0600` in a `0700` parent dir |

`<config-dir>` resolves to `~/.pi/`.
