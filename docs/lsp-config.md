# LSP configuration in Pi

This guide explains how to configure language servers for the Pi coding agent.
...
When no LSP config file is present, Pi auto-detects servers by intersecting two conditions:
...
Pi merges LSP config from multiple files, lowest to highest priority:

| Priority    | Location                                                                                                                    |
| ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| 5 (lowest)  | `~/lsp.json`, `~/.lsp.json`, `~/lsp.yaml`, `~/.lsp.yaml`, `~/lsp.yml`, `~/.lsp.yml`                                         |
| 4           | Plugin LSP configs (marketplace / `--plugin-dir` roots)                                                                     |
| 3           | User config dirs: `~/.pi/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.pi/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.pi/agent/lsp.json`
- Project-specific overrides → `<project>/.pi/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets Pi auto-detect built-in servers. When server overrides exist, Pi merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that Pi supports on a per-server basis:
...
Place the override in `<project>/.pi/lsp.json`:
...
The user-level config in `~/.pi/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
