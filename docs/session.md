...
```text
~/.pi/agent/sessions/<dir-encoded>/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.pi/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.pi/agent/terminal-sessions/<terminal-id>
```
...
- `listAllSessions(storage)` (a.k.a. `SessionManager.listAll()`) -> sessions across all project scopes under `~/.pi/agent/sessions`
