...
└───────────────────────────────────────┘
 ...
 (native provider scans project .pi before user .pi;
...
`CONFIG_DIR_NAME` is `.pi` (`packages/utils/src/dirs.ts`).

## Profiles

A named profile (`pi --profile <name>`, the `--alias` shortcut, or `PI_PROFILE`) relocates the Pi user base. When a profile is active, every Pi-native user-level path written here as `~/.pi/agent/...` resolves to `~/.pi/profiles/<name>/agent/...` instead.

The relocation is uniform across the native provider (`builtin.ts`) and the generic `config.ts` helpers, so it covers slash commands, rules, prompts, instructions, hooks, tools, extensions, settings, skills, and MCP, plus the top-level `SYSTEM.md` / `RULES.md` / `AGENTS.md` files and runtime state (sessions, blobs, `agent.db`). A profile sees only its own Pi config, never the default profile's `~/.pi/agent`.

The other source bases are not profile-scoped and load identically under every profile: the external-tool bases (`~/.claude`, `~/.codex`, `~/.gemini`) belong to those tools, and the project-level bases (`<cwd>/.pi`, `<cwd>/.claude`, ...) are keyed to the working directory. Throughout this document, read `~/.pi/agent` as shorthand for the active profile's agent directory.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.pi`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.pi/agent/config.yml`
2. Project settings: discovered via settings capability (`settings.json` and `config.yml` from providers)
3. CLI config overlays: `pi --config <path>` / repeated `--config` files, loaded as `config.yml`-style YAML for this process only
...
1. Migrate from `~/.pi/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.pi` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.pi/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.pi/agent/skills`, without requiring the root `.pi` directory itself to be non-empty.
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.pi` lookup for project files, but the project `.pi` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.pi/skills/*/SKILL.md` and `~/.pi/agent/skills/*/SKILL.md`
...
## For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.pi` directory search (walk-up) and still requires the project `.pi` dir to be non-empty.
...
```text
# ~/.pi/agent/TITLE_SYSTEM.md
Generate a session name using lowercase `<type>:<primary-objective>`.
```

- Missing `TITLE_SYSTEM.md` keeps the bundled title prompts.
- Discovery uses the same project-then-user config directory pattern as `SYSTEM.md`: project `.pi/TITLE_SYSTEM.md` first, then user `~/.pi/agent/TITLE_SYSTEM.md` and the other supported config bases.
