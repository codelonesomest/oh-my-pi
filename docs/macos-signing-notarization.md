...
The compiled macOS `pi` binaries shipped on GitHub Releases are signed with a
...
- `curl https://pi.sh/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/pi-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/pi-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/pi-signing             # upload all five
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/pi-darwin-arm64
```
