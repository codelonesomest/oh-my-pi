...
- Project directory: `<cwd>/.pi/extensions`
- User directory: `~/.pi/agent/extensions`
- Native settings JSON entries: `<cwd>/.pi/settings.json#extensions` and `~/.pi/agent/settings.json#extensions`

The project root is the native provider's `.pi` directory (`SOURCE_PATHS.native.projectDir`), cwd-only; it does not walk ancestors. The user root is the active profile's agent directory via `getAgentDir()`, so under `pi --profile <name>` it becomes `~/.pi/profiles/<name>/agent/extensions` . See [Profiles](./config-usage.md#profiles).

Notes:

- Native auto-discovery is `.pi` based.
...
Plugin extension entries come from package `pi.extensions` manifests, including enabled feature entries.
...
- User: `~/.pi/agent/config.yml` (or the active profile's agent dir)
- Project/native settings capability: `<cwd>/.pi/config.yml` and `<cwd>/.pi/settings.json`
...
```json
{
  "extensions": ["./.pi/extensions/my-extra"]
}
```
...
1. `package.json` in that directory with `pi.extensions` -> use declared entries
2. `index.ts`
3. `index.js`
4. Otherwise scan one level for extension entries:
   - direct `*.ts` / `*.js`
   - subdir `index.ts` / `index.js`
   - subdir `package.json` with `pi.extensions`
