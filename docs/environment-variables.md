...
3. Agent `.env` (`~/.pi/agent/.env`, or `~/.pi/profiles/<name>/agent/.env` under `PI_PROFILE`) for keys not already set
4. Config-root `.env` (`~/.pi/.env`) for keys not already set
...
| Variable | Used for | Required when | Notes / precedence |
| --- | --- | --- | --- |
| `PI_AUTH_BROKER_URL` | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`); selects broker mode | Resolving credentials through a broker; also required by `pi auth-gateway serve` (the gateway is itself a broker client) | Wins over `auth.broker.url` in `config.yml`. When set with no resolvable token, `resolveAuthBrokerConfig()` hard-errors instead of falling back to local SQLite. Legacy fallback: `OMP_AUTH_BROKER_URL`. |
| `PI_AUTH_BROKER_TOKEN` | Bearer token sent on every broker endpoint except `/v1/healthz` | `PI_AUTH_BROKER_URL` is set and no token is available from `auth.broker.token` or `<config-dir>/auth-broker.token` | Resolution: this env → `auth.broker.token` (`$ENV_NAME` indirection supported) → `<config-dir>/auth-broker.token` (mode `0600`). `<config-dir>` is `~/.pi/`. Legacy fallback: `OMP_AUTH_BROKER_TOKEN`. |
| `PI_AUTH_BROKER_SNAPSHOT_TTL_MS` | Freshness window for the encrypted local broker snapshot cache | Optional in broker mode | Default `3600000` (1 h). Freshness is based on broker `snapshot.generatedAt`; `0` disables cache reads/writes and forces the old blocking fetch every startup. Legacy fallback: `OMP_AUTH_BROKER_SNAPSHOT_TTL_MS`. |
| `PI_AUTH_BROKER_SNAPSHOT_CACHE` | Path to the encrypted local broker snapshot cache | Optional in broker mode | Defaults to `~/.pi/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Useful for tests, ephemeral hosts, or relocating the `0600` cache file. Legacy fallback: `OMP_AUTH_BROKER_SNAPSHOT_CACHE`. |

The gateway has no dedicated env vars — it inherits `PI_AUTH_BROKER_*` (with `OMP_AUTH_BROKER_*` recognized only as a legacy fallback). Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `pi auth-gateway token`.
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.pi/agent/config.yml`; environment variables are fallbacks.
...
| `PI_SUBPROCESS_CMD`          | Overrides subagent spawn command (`pi` / `pi.cmd` resolution bypass)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `OLLAMA_CONTEXT_LENGTH`      | Positive integer context-window override for implicit Ollama discovery; affects Pi context budgeting only and does not change Ollama's runtime `num_ctx`                                                                                                                                                                                                                                                                                                                                     |
...
| `PI_MCP_TIMEOUT_MS`          | Overrides MCP client request timeout (ms) for every MCP server. `0` disables client-side timeouts (`AbortSignal` never fires). Invalid (negative or non-numeric) values are ignored with a warning and the per-server config or default (`30000`) is used. Legacy fallback: `OMP_MCP_TIMEOUT_MS`.                                                                                                                                                                                                                                                                                                                                                 |
...
| Variable              | Default / behavior                                                            |
| --------------------- | ----------------------------------------------------------------------------- |
| `PI_PROFILE` | Selects the active named profile; unset uses the default profile |
